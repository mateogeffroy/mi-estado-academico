...
import { getInhabiles } from '../lib/data/calendario'; // 🔥 Importamos la nueva función inteligente
import { usePlan } from '../context/PlanContext'; // 🔥 Importamos el contexto
...
export default function HorarioCalendar({ horarios, isEmpty, title, action, detalles, materiasData }: HorarioCalendarProps) {
  ...
  // 🔥 Extraemos la carrera activa y generamos los feriados específicos para ella 🔥
  ...
  // Estados del Modal del Calendario Mensual
  ...
  // --- Lógica del Calendario Mensual (Modal) ---
  ...
  const modalDays = [];
  for (let i = 0; i < startDay; i++) modalDays.push(null);
  for (let i = 1; i <= daysInMonth; i++) modalDays.push(i);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <style>{`
        ...
        .cm-day:hover:not(.empty) { background: var(--glass-hover); border-color: var(--border); }
        .cm-day.empty { visibility: hidden; pointer-events: none; }
        .cm-day.selected { border-color: var(--cursando) !important; background: rgba(59, 130, 246, 0.1) !important; color: var(--cursando); }
        .cm-day.today { background: var(--cursando) !important; color: #fff !important; box-shadow: 0 4px 10px rgba(59, 130, 246, 0.3); border-color: transparent !important; }
      ...
      <div className={`modal-overlay ${isModalOpen ? 'open' : ''}`} onClick={() => setIsModalOpen(false)}>
        <div className="calendar-modal" onClick={e => e.stopPropagation()}>
          ...
          <div className="cm-grid">
            ...
            {modalDays.map((dia, idx) => {
              if (dia === null) return <div key={`empty-${idx}`} className="cm-day empty" />;
              
              const dateStr = `${currentYear}-${String(currentMonth + 1).padStart(2, '0')}-${String(dia).padStart(2, '0')}`;
              ...
              return (
                <div 
                  key={idx} 
                  className={`cm-day ${isToday ? 'today' : ''} ${isSelected && !isToday ? 'selected' : ''}`}
                ...
                >
                  ...
                  {evs.length > 0 && (
                    <div className="cm-dots">
                      {evs.map((e, i) => (
                        <div key={i} className="cm-dot" style={{ background: isToday ? '#fff' : getEventColor(e.tipo) }} />
                      ))}
                    </div>
                  )}
                </div>
      ...
      <div className="hc-header-wrapper">
        ...
        <div className="hc-nav-box">
          ...
          <div className="hc-nav-text">
            <span style={{ color: 'var(--text-strong)', fontWeight: '800', fontSize: '0.95rem', textAlign: 'center', whiteSpace: 'nowrap' }}>
