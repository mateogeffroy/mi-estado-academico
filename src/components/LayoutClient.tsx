...
export default function LayoutClient({ children }: { children: React.ReactNode }) {
  ...
  // Saber si el usuario tiene sesión activa para armar el menú
  ...
  // --- LÓGICA DE SESIÓN BLINDADA CON RUTAS PÚBLICAS ---
  useEffect(() => {
    let isMounted = true; 

    // Filtro a prueba de balas para rutas públicas (soporta sub-rutas y trailing slashes)
    ...
    const checkAccess = async () => {
      try {
        ...
        if (!session) {
          // Si NO hay sesión y NO es pública, lo pateamos al login
          if (!isPublicRoute) {
            router.replace('/login');
          } else {
            setIsChecking(false); // Es pública, lo dejamos leer tranquilo
          }
        } else {
          // Si HAY sesión y está en el login, lo mandamos al inicio
          if (pathname === '/login') {
            router.replace('/');
          } else {
            setIsChecking(false);
        ...
        <div style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px', position: 'relative', zIndex: 1000 }}>
          ...
          <div style={{ flex: 1, display: 'flex', justifyContent: 'flex-end', alignItems: 'center' }}>
            <div className="nav-full-menu" style={{ gap: '10px', alignItems: 'center' }}>
              ...
              {pathname === '/plan' && hasSession && (
                <button className="help-btn" onClick={() => setIsModalOpen(true)} title="Ver instrucciones del Plan" style={{ marginRight: '6px' }}>
              ...
              {hasSession ? (
                <>
                  ...
                  {pathname === '/' && (
                    <button className="help-btn" onClick={() => window.dispatchEvent(new Event('start-home-tour'))} title="Guia de inicio" style={{ marginRight: '6px' }}>
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
                    </button>
                  )}
                  ...
                  {pathname === '/' && (
                    <Link href="https://cafecito.app/mateogeffroy" target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none' }}>
                      <button style={{ ...navBtnBase, background: 'rgba(245, 158, 11, 0.1)', color: '#f59e0b', border: '1px solid rgba(245, 158, 11, 0.2)' }} onMouseOver={(e) => { e.currentTarget.style.background = '#f59e0b'; e.currentTarget.style.color = 'black'; }} onMouseOut={(e) => { e.currentTarget.style.background = 'rgba(245, 158, 11, 0.1)'; e.currentTarget.style.color = '#f59e0b'; }}>
                ...
                /* OPCIONES SI EL USUARIO NO ESTÁ LOGUEADO (Invitado en Blog) */
      ...
      <aside className={`sidebar ${isSidebarOpen ? 'active' : ''}`}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            ...
            {hasSession ? (
              <>
                ...
                {pathname === '/' && (
                  <>
                    <button onClick={() => { setIsSidebarOpen(false); window.dispatchEvent(new Event('start-home-tour')); }} className="sidebar-action-btn sidebar-action-btn-custom" style={{ color: 'var(--cursando)', borderColor: 'var(--border)', background: 'transparent', display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        {pathname === '/plan' && hasSession && (
          <div className="sidebar-legend">
            ...
            <button className="sidebar-action-btn sidebar-action-btn-custom" onClick={() => { setIsSidebarOpen(false); setIsModalOpen(true); }} style={{ justifyContent: 'center', background: 'var(--cursando)', color: 'black', border: 'none', marginTop: '4px' }}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
              ¿Cómo usar el Plan?
            </button>
          </div>
      ...
      <footer style={{ background: 'var(--panel)', borderTop: '1px solid var(--border)', padding: '30px 20px', marginTop: 'auto' }}>
        <div style={{ width: '100%', maxWidth: '1200px', margin: '0 auto', display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: '20px' }}>
          ...
          <div style={{ display: 'flex', gap: '15px', justifyContent: 'center', alignItems: 'center', flex: '1 1 auto', fontSize: '0.8rem', color: 'var(--muted)' }}>
            <Link href="/terminos" style={{ color: 'inherit', textDecoration: 'none', transition: 'color 0.2s' }} onMouseOver={(e) => e.currentTarget.style.color = 'white'} onMouseOut={(e) => e.currentTarget.style.color = 'var(--muted)'}>
              Términos y Condiciones
            </Link>
            <span style={{ opacity: 0.5 }}>|</span>
            <Link href="/privacidad" style={{ color: 'inherit', textDecoration: 'none', transition: 'color 0.2s' }} onMouseOver={(e) => e.currentTarget.style.color = 'white'} onMouseOut={(e) => e.currentTarget.style.color = 'var(--muted)'}>
